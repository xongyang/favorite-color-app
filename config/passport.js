// This file holds the authentication configuration information
...
module.exports = function(passport) {
    ...
    // allow user to use twitter to sign in
    passport.use(new TwitterStrategy({
        ...
        process.nextTick(function() {
            // find document with the same twitter id
            User.findOne( {'twitter.id' : profile.id}, function (err, user) {

                if(err) {return done(err);} // Return database error

                if(user) {return done(null, user)} // Return user if already exist in the database

                // If user is not in the database, create a document
                // containing their Twitter authentication information and profile
                var newUser = User();
                ...
                // Save User document, return err if there is an error
                // or return the new User document with user's twitter info
                newUser.save( function(err) {
                    if(err) {return done(err);}
                    return done(null, newUser);
                })
